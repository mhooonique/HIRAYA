...
class _SearchScreenState extends ConsumerState<SearchScreen>
  ...
  void _onQueryChanged(String val) {
    ...
    _debounce = Timer(const Duration(milliseconds: 350), () {
      ...
      if (val.isNotEmpty) ref.read(searchProvider.notifier).setQuery(val);
    });
  }
...
class _SearchHeader extends StatelessWidget {
  ...
  const _SearchHeader({
    required this.queryCtrl, required this.focusNode,
    required this.onChanged, required this.onSubmit,
    required this.onFilterTap, required this.activeFilters,
    required this.state,
  });
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(
            child: Container(
              ...
              child: TextField(
                ...
                style: const TextStyle(fontSize: 14, color: AppColors.darkGray, fontFamily: 'Poppins'),
                decoration: InputDecoration(
                  hintText: 'Search innovations, categories...',
                  hintStyle: TextStyle(fontSize: 13, color: Colors.grey.shade400, fontFamily: 'Poppins'),
                  prefixIcon: const Icon(Icons.search, color: AppColors.navy, size: 20),
                  suffixIcon: queryCtrl.text.isNotEmpty
                      ? IconButton(
                          icon: const Icon(Icons.clear, size: 18),
                          onPressed: () { queryCtrl.clear(); onChanged(''); })
                      : null,
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: Icon(Icons.tune_rounded,
                    color: activeFilters > 0 ? AppColors.navy : Colors.white, size: 22),
              ),
            ),
            if (activeFilters > 0)
              Positioned(
        ...
        if (state.totalResults > 0) ...[
          ...
          Row(children: [
            Text('${state.totalResults} result${state.totalResults == 1 ? '' : 's'}',
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          ..._sortLabels.entries.map((e) => ListTile(
                leading: Icon(
                  current == e.key ? Icons.radio_button_checked : Icons.radio_button_off,
                  color: current == e.key ? AppColors.crimson : Colors.grey,
                ),
                title: Text(e.value, style: TextStyle(fontFamily: 'Poppins',
                    fontWeight: current == e.key ? FontWeight.w600 : FontWeight.normal)),
                onTap: () {
        ...
        children: suggestions.take(5).map((s) {
          ...
          return ListTile(
            ...
            title: Text(s, style: const TextStyle(fontSize: 13,
                color: AppColors.darkGray, fontFamily: 'Poppins')),
            onTap: () => onTap(s),
          );
        }).toList(),
      ),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (state.searchHistory.isNotEmpty) ...[
          ...
          Wrap(
            ...
            children: state.searchHistory.take(8).map((h) => _HistoryChip(
                  label: h,
                  onTap: () => onSearch(h),
                  onRemove: () =>
                      ref.read(searchProvider.notifier).removeHistoryItem(h),
                )).toList(),
          ),
        ...
        if (state.trendingTopics.isNotEmpty) ...[
          ...
          ...state.trendingTopics.take(6).map((t) => _TrendingTopicTile(
                topic: t,
                onTap: () =>
                    ref.read(searchProvider.notifier).searchTrending(t.keyword),
              )),
          const SizedBox(height: 24),
        ],
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, i) => SizedBox(
                width: 220,
                child: ProductCard(product: state.trendingProducts[i], index: i),
              ),
            ),
          ),
        ],
...
class _TrendingTopicTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      leading: Container(
        width: 36, height: 36,
        decoration: BoxDecoration(color: AppColors.navy.withOpacity(0.08),
            borderRadius: BorderRadius.circular(8)),
        child: const Icon(Icons.trending_up, color: AppColors.navy, size: 18),
      ),
      title: Text(topic.keyword, style: const TextStyle(fontSize: 13,
    ...
    return CustomScrollView(
      slivers: [
        if (state.filters.hasActiveFilters)
          SliverToBoxAdapter(child: _ActiveFilterChips(filters: state.filters)),
        SliverPadding(
          padding: const EdgeInsets.all(16),
          sliver: SliverGrid(
            delegate: SliverChildBuilderDelegate(
              (ctx, i) {
                ...
                return ProductCard(product: state.results[i], index: i);
              },
              childCount: state.results.length + 1,
            ),
            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: isWide ? 4 : 2,
              mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 0.65,
            ),
          ),
        ),
      ],
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (filters.category != null)
      chips.add(_Chip(
          ...
          onRemove: () => ref.read(searchProvider.notifier)
              .updateFilters(filters.copyWith(clearCategory: true))));
    if (filters.sortBy != 'trending')
      chips.add(_Chip(
          label: 'Sort: ${filters.sortBy}',
          onRemove: () => ref.read(searchProvider.notifier).setSortBy('trending')));
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      ...
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
          ...
          crossAxisSpacing: 12, childAspectRatio: 0.65),
      itemCount: 6,
...
class _FilterSheetState extends ConsumerState<_FilterSheet> {
  ...
  void _apply() {
    ref.read(searchProvider.notifier).updateFilters(_local);
    Navigator.pop(context);
  }
        ...
        child: Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              TextButton(
                onPressed: () => setState(() =>
                    _local = _local.copyWith(clearCategory: true)),
            ...
            child: ListView(
              ...
              children: [
                Wrap(
                  ...
                  children: _categories.map((c) => _SelectableChip(
                        label: c,
