...
// ── Provider: fetches product directly — supports pending/rejected too ─────────
final _productDetailProvider =
    FutureProvider.family<ProductModel?, int>((ref, id) async {
  final api = ref.read(apiServiceProvider);
  // First try marketplace cache (already has images parsed)
  final cached = ref
      .read(marketplaceProvider)
      .products
      .cast<ProductModel?>()
      .firstWhere((p) => p?.id == id, orElse: () => null);
  // Always fetch directly to get video_base64 and support non-approved products
  try {
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  // Status banner for non-approved products
                            ...
                            children: [
                              Text(product.innovatorName,
                                  style: TextStyle(fontFamily: 'Poppins',
                                      ...
                                      color: primaryText)),
                              Text('@${product.innovatorUsername}',
                                  style: TextStyle(fontFamily: 'Poppins',
                  ...
                  Container(
                    ...
                    child: Text(product.description,
                        style: TextStyle(fontFamily: 'Poppins', fontSize: 14,
                            color: secondaryText, height: 1.7)),
                  ...
                  GridView.count(
                    ...
                    children: [
                      _DetailChip(icon: Icons.category_rounded,
                          label: 'Category', value: product.category,
                          color: color),
                      _DetailChip(icon: Icons.verified_user_rounded,
                          label: 'KYC Status',
                          value: product.kycStatus.toUpperCase(),
                          color: product.isVerifiedInnovator
                              ? AppColors.teal : AppColors.golden),
                      _DetailChip(icon: Icons.calendar_today_rounded,
                          label: 'Listed',
                          value: '${product.createdAt.day}/${product.createdAt.month}/${product.createdAt.year}',
                          color: AppColors.sky),
                      _DetailChip(icon: Icons.bar_chart_rounded,
                          label: 'Status',
        ...
        child: Row(children: [
          ...
          Expanded(
            child: ElevatedButton.icon(
              ...
              icon: Icon(
                _interestSent ? Icons.check_rounded : Icons.handshake_rounded,
                color: Colors.white, size: 18,
              ),
              label: Text(
                _interestSent ? 'Interest Sent!' : 'Express Interest',
                style: const TextStyle(fontFamily: 'Poppins',
        ...
        children: [
          ...
          Text(product.name,
              style: const TextStyle(fontFamily: 'Poppins', fontSize: 28,
                  ...
                  height: 1.2)).animate().fadeIn(duration: 500.ms),
          const SizedBox(height: 16),
          Row(children: [
            _HeroStat(icon: Icons.favorite_rounded,
                value: '${product.likes}', label: 'Likes'),
            const SizedBox(width: 20),
            _HeroStat(icon: Icons.remove_red_eye_rounded,
                value: '${product.views}', label: 'Views'),
            const SizedBox(width: 20),
            _HeroStat(icon: Icons.trending_up_rounded,
                value: '${product.interestCount}', label: 'Interests'),
          ]),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        const SizedBox(width: 4),
        Text(value, style: const TextStyle(fontFamily: 'Poppins', fontSize: 18,
...
class _SectionTitle extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final isDark = ref.watch(themeProvider) == ThemeMode.dark;
    return Text(title, style: TextStyle(fontFamily: 'Poppins', fontSize: 18,
